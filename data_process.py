...
open(VAL_SAMPLE_PATH, 'w', encoding='utf-8').close()
...
process_and_save_split(dataset['validation'], VAL_SAMPLE_PATH)
process_and_save_split(dataset['test'], TEST_SAMPLE_PATH)

# Generate vocab
def generate_vocab():
    vocab_set = set()
    # Include validation data in vocab generation
    for file_path in [TRAIN_SAMPLE_PATH, VAL_SAMPLE_PATH]:
